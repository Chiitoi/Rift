import { EVENTS } from '#constants'
import { ApplyOptions } from '@sapphire/decorators'
import { Listener, type ListenerOptions, type UserError } from '@sapphire/framework'
import type { CommandInteraction, CommandInteractionOptionResolver, MessageEmbed } from 'discord.js'

@ApplyOptions<ListenerOptions>({ event: EVENTS.INTERACTION_ERROR })
export class RiftListener extends Listener {
    public async run({ message }: UserError,{ interaction, options }: { interaction: CommandInteraction, options: Omit<CommandInteractionOptionResolver<'cached'>, 'getMessage' | 'getFocused'> }) {
        const embed: Partial<MessageEmbed> = { color: 0xF8F8FF, description: message }

        if (interaction.deferred)
            await interaction.editReply({ embeds: [embed] })    
        else
            await interaction.reply({ embeds: [embed], ephemeral: true })
    }
}